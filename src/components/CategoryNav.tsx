...
export default function CategoryNav({
  ...
  const isCategoryActive = (cat: Category) => {
    if (cat.id === 'all-categories' || cat.name === 'All Categories') {
      return (
        !selectedCategory ||
        selectedCategory === '' ||
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={`group/item flex flex-col items-center justify-center min-w-[68px] sm:min-w-[84px] md:min-w-[92px] py-1 px-1.5 sm:px-2 rounded-xl transition-all cursor-pointer shrink-0 border border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5A00] ${
                  active
                    ? 'text-[#FF5A00]'
              ...
              >
                ...
                >
                  <CategoryIcon iconKey={cat.icon} className="w-5 h-5 sm:w-6 sm:h-6 shrink-0 transition-transform group-hover/item:scale-110" />
                </div>

                {/* Category Name */}
                <span
                  className={`mt-1 text-[10px] sm:text-[12px] md:text-[13px] text-center whitespace-nowrap tracking-tight leading-tight max-w-[80px] sm:max-w-[100px] truncate ${
                    active
                ...
                <div
                  className={`h-0.5 rounded-full mt-1 transition-all duration-200 ${
                    active ? 'w-6 sm:w-8 bg-[#FF5A00]' : 'w-0 bg-transparent'
                  }`}
                />
              </button>
            );
